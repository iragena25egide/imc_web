...
import { Plus, Trash2, Image as ImageIcon, Video, X, UploadCloud, Edit, Search, ArrowUpDown, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function AdminGalleryPage() {
  ...
  const filteredItems = items.filter((item: any) => {
    const itemTitle = item.title || (item.type === 'video' ? "Untitled Video" : "Untitled Image");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const toastId = toast.loading(isEditing ? "Updating media..." : "Uploading media...");
    ...
    try {
      ...
      // If uploading new file (create or edit)
      ...
      // We'll just create a new one if file is provided, because backend upload creates new.
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-slate-800">Gallery Media</h1>
        <button 
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                <UploadCloud size={24} className="text-imc-blue" /> {editingId ? "Edit Media Title" : "Upload Media"}
              </h2>
              <button onClick={() => { setIsAdding(false); setFile(null); setPreview(null); setEditingId(null); }} className="text-slate-400 hover:text-slate-600 p-1">
                <X size={20} />
              </button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div>
                ...
                <input 
                  ...
                  placeholder="E.g., Mining Site 2026"
                  className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-imc-blue outline-none text-slate-800 placeholder-slate-400"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">Media File</label>
                {preview ? (
                  <div className="relative w-full h-48 bg-slate-100 rounded-xl overflow-hidden border border-slate-200 mb-3 group">
                ...
                {editingId && <p className="text-xs text-slate-500">Note: To change the media file, please delete this item and create a new one.</p>}
              </div>
              <div className="pt-4 flex justify-end gap-3 border-t border-slate-100">
                <button type="button" onClick={() => { setIsAdding(false); setFile(null); setPreview(null); setEditingId(null); }} className="px-5 py-2.5 text-slate-600 hover:bg-slate-100 rounded-xl font-medium transition-colors">
                  Cancel
                </button>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <table className="w-full">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              <th className="px-6 py-4 text-left text-sm font-semibold text-slate-600">Media</th>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        ...
                        <span className="text-slate-800 font-medium">
                          {item.title || (item.type === 'video' ? "Untitled Video" : "Untitled Image")}
                        </span>
                      </div>
                    </td>
                ...
                {paginatedItems.length === 0 && (
                  <tr>
                    <td colSpan={3} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-slate-500 font-medium text-lg mb-1">No media found</p>
                    </td>
                  </tr>
                )}
              </>
